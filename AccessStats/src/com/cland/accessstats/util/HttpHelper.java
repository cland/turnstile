...
public class HttpHelper {
   ...
   public String performGet(final String url,Map<String,String>params ){
	   return performRequest(null, url, null, null, null, params, HttpHelper.GET_TYPE);
   }
   /**
   ...
   /**
    * Perfom an HTTP GET operation with a cookiestore as parameter for session authentication. 
    * Tested on a domino server used in conjuction with performCustomPost method for login
    * @param url
    * @param cookieStore
    * @return
    */
   public String performCustomGet(final String url, final CookieStore cookieStore){
   ...
   /** 
    * Perfom an HTTP Post operation. Difference is that this function was tested on domino http authentication and works
    * using contentType: "application/x-www-form-urlencoded" and the standard domino "/names.nsf?login" url format.
    * @param contentType
    * @param url
    * @param username
    * @param password
    * @return
    */
   public String performCustomPost(final String contentType, final String url, final String username, final String password)
	{		
		String result = "";
	    ...
	    try
	    {
	        ...
	        HttpResponse response = HttpHelper.client.execute(post);
	        result = response.getStatusLine().getReasonPhrase();
	    }
	    catch (ClientProtocolException e) {
	         result = HttpHelper.HTTP_RESPONSE_ERROR + " - " + e.getClass().getSimpleName() + " " + e.getMessage();
	         //e.printStackTrace();
	      } catch (IOException e) {
	         result = HttpHelper.HTTP_RESPONSE_ERROR + " - " + e.getClass().getSimpleName() + " " + e.getMessage();
	         //e.printStackTrace();
   ...
   public CookieStore getCookieStore(){
	   return new BasicCookieStore();//  HttpHelper.client.getCookieStore();
   }
...
//         });
   ...
   private synchronized String execute(final HttpRequestBase method, final HttpContext context){
	   String response = null;
	   try {
	         response = HttpHelper.client.execute(method, responseHandler,context);
	      } catch (ClientProtocolException e) {
